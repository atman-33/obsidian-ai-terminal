...
Enable users to launch AI terminals with custom prompts directly from context menus without creating command templates. This capability provides a dialog-based interface for ad-hoc queries with visible context placeholders, agent selection, and flexible prompt construction.

This specification also defines centralized agent management that is shared across direct prompt input and command templates.
